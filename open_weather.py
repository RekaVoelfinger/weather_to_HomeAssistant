...
def get_weather_acw(api_key, location_key):
...
def main():
    ...
    forecast_ow = get_weather_ow(api_key_ow, "48.45", "9.89")
    ...
    # AccuWeather
    api_key_acw = api_helper.get_api_key("accuweather")
    # location_key = "986294"  # Weidach
    location_key = api_helper.get_location_key(api_key_acw, "48.45", "9.89")
    forecast_acw = get_weather_acw(api_key_acw, location_key)
    ...
    print(f"Rainy hours: {will_it_rain_acw(forecast_acw, time_to_int(time_of_query_acw), 12)}") #TODO time to clarify by the different services
